...
const NewUsers: React.FC<NewUsersProps> = ({ onSelectUser }) => {
  ...
  const fetchNewUsers = useCallback(async (isRefresh = false) => {
    // 先检查全局缓存管理服务中的预加载数据
    const globalCacheKey = `new_users_${currentUser?.id || 'unknown'}`;
    console.log('[NewUsers] 检查全局缓存，键:', globalCacheKey);
    const globalCachedData = cacheManager.get(globalCacheKey, 300000); // 5分钟全局缓存
    console.log('[NewUsers] 全局缓存数据:', globalCachedData ? `存在(${globalCachedData.users?.length || 0}条用户)` : '不存在');
    ...
    console.log('[NewUsers] 检查本地缓存，键:', cacheKey);
