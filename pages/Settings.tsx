...
const Settings: React.FC<SettingsProps> = ({ onLogout }) => {
  ...
  const fetchEarnings = async () => {
    try {
      ...
      if (isTeamLeader && currentUser?.teamName) {
        ...
        // 获取团队累计金币
        const totalResponse = await request<any>(`/dashboard/kpi?range=all&team=${encodeURIComponent(currentUser.teamName)}`, {
          method: 'GET',
          headers: new Headers({
            'Content-Type': 'application/json'
          })
  ...
  const loadWithdrawStatus = async () => {
    try {
      const result = await request<any>('/withdraw/status', {
        method: 'GET',
        headers: new Headers({
          'Content-Type': 'application/json'
        })
            ...
            <div className="space-y-4">
              {withdrawSuccess ? (
                <div className="text-center py-6">
                  ...
                  <h4 className="text-lg font-black text-gray-900 mb-2">提现申请已提交</h4>
                ...
                <>
                  ...
                  <div className="bg-purple-50 p-4 rounded-2xl">
                    <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">可提现金额</div>
                    <div className="text-2xl font-black text-purple-600">¥{earnings.lastMonth.toFixed(2)}</div>
                    <div className="text-[10px] text-gray-400 mt-1">
                      {(earnings.lastMonth * 1000).toFixed(0)} 金币 ÷ 1000 = ¥{earnings.lastMonth.toFixed(2)}
                    </div>
                  </div>
                  <div>
                  ...
                  <p className="text-[10px] text-gray-400 text-center">提现申请将在3个工作日内处理</p>
                </>
              )}
              <div className="flex space-x-3 pt-2">
                <button
                ...
                {!withdrawSuccess && (
                  <button
                    ...
                    className={`flex-1 py-3 text-xs font-black rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center ${
                      isWithdrawing || !alipayAccount.trim() || !alipayName.trim() || earnings.lastMonth <= 0
                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                        : 'bg-purple-600 text-white shadow-purple-100'
                    }`}
                  >
                    {isWithdrawing ? <Loader2 size={16} className="animate-spin" /> : '提交提现申请'}
                  </button>
                )}
              </div>
            </div>
